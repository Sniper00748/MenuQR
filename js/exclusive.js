document.addEventListener('DOMContentLoaded', function() {
    ...
    // 加载来生专属菜品
    function loadExclusiveCocktails() {
        menuItemsContainer.innerHTML = '';
        
        // 筛选出来生专属类别的菜品 (categoryId = 2)
        const items = menuData.items.filter(item => item.categoryId === 2);
    ...
    function generateRadarChart(stats) {
        ...
        for (let level = 1; level <= 5; level++) {
            ...
            gridLines += `<polygon points="${levelPoints.join(' ')}" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="0.5"/>`;
        }
        ...
        for (let i = 0; i < 6; i++) {
            ...
            axisLines += `<line x1="${center}" y1="${center}" x2="${point.x}" y2="${point.y}" stroke="rgba(255,255,255,0.3)" stroke-width="0.5"/>`;
        }
        ...
        for (let i = 0; i < 6; i++) {
            ...
            labels_svg += `<text x="${labelX}" y="${labelY + 2}" text-anchor="${textAnchor}" fill="#e0e0e0" font-size="8" font-family="Microsoft YaHei">${point.label}</text>`;
        }
        ...
        return `
            <svg viewBox="-15 -15 130 130" xmlns="http://www.w3.org/2000/svg">
                ${gridLines}
                ${axisLines}
                <path d="${dataPath}" fill="rgba(138, 43, 226, 0.3)" stroke="rgba(138, 43, 226, 0.8)" stroke-width="1.5"/>
                ${dataPoints.map(point => `<circle cx="${point.x}" cy="${point.y}" r="1.5" fill="rgba(138, 43, 226, 1)"/>`).join('')}
                ${labels_svg}
            </svg>
        `;
    }
